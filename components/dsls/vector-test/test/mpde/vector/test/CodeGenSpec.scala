    ...
    if (!noGuardCountAssertFlag) {
      ...
      if (runtimeCompileCount != 0) {
        if (guardCountPrintDetails > 1) {
          scala.Predef.printf("--t: %6dµs, dt: %5dµs, c: %3d, dc: %3d, dt/dc: %5dµs, ct: %2d, rt: %2d\n", time, time - guardTime, count, count - guardCount,
            (time - guardTime) / (count - guardCount), compileTimeCompileCount, runtimeCompileCount)
        }
        var minGuardTime = ((time - guardTime) / (count - guardCount), run2)
        for (i <- 0 until 5) {
          guardTime = time
          guardCount = count
          ...
          if (guardCountPrintDetails > 1) {
            scala.Predef.printf("---t: %5dµs, dt: %5dµs, c: %3d, dc: %3d, dt/dc: %5dµs, ct: %2d, rt: %2d\n", time, time - guardTime, count, count - guardCount,
              (time - guardTime) / (count - guardCount), compileTimeCompileCount, runtimeCompileCount)
          }
        ...
        if (guardCountPrintDetails > 0) {
          scala.Predef.printf( /* "--Min. execution time in microseconds for guard check: " */
            "%5dµs (%2d checks, %2d initial compilCount, %2d minimal compilCount)\n",
            minGuardTime._1, count - guardCount, runtimeCompileCount, minGuardTime._2)
        }
      }
    }
  }
